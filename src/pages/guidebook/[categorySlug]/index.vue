...
<!-- <template>
  <GuidebookCategoryView :category="category" />
</template>
...
import { useListCategories } from '~/composables/blogCategory/useListCategories';
import { useTypedRoute } from '~/composables/useTypedRoute';
import GuidebookCategoryView from '~/page-views/GuidebookCategoryView.vue';
import { ToGuidebookCategory } from '~/utils/routes';

const { data: categories } = await useListCategories({
  params: { articles: true },
});

const route = useTypedRoute<ToGuidebookCategory>();
