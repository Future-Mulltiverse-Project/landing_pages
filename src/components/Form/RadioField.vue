<template>
  <div class="form-group" :class="$style.group">
    <div class="row" :class="$style.row">
      <div class="col-auto">
        <label
          :for="name + displayOptions[0]?.value"
          :class="[
            'form-label',
            $style.label,
      ...
      <div class="col-auto">
        <div class="row" :class="$style.radiosRow">
          <div v-for="option in displayOptions" :key="option" class="col-auto">
            <div class="form-check">
              <input
                ...
                @input="handleChange"
              />
              <label class="form-radio-label" :for="name + option.value">
                {{ option.label }}
              </label>
            </div>
          </div>
...
.group {
  padding: rem-calc(35px 0 62px);
}

.row {
  --gutter-x: #{rem-calc(50px)};
  --gutter-y: #{rem-calc(12px)};
}

.radiosRow {
  --gutter-x: #{rem-calc(36px)};
